...
def quantum_die(p1_place, p2_place, spaces, faces, to_win):
    ...
    def take_turn(p1_score, p2_score, p1_place, p2_place, p1_turn):
        if p1_score >= to_win and p1_turn:
            wins[0] += 1
            return
        if p2_score >= to_win and not p1_turn:
            wins[1] += 1
            return
            ...
            for roll2 in range(1, faces + 1):
                for roll3 in range(1, faces + 1):
                    ...
                    if p1_turn:
                        ...
                        p2_place %= spaces
                        p2_score += p2_place + 1
                    take_turn(p1_score, p2_score, p1_place, p2_place, not p1_turn)
    
    take_turn(0, 0, p1_place, p2_place, True)
